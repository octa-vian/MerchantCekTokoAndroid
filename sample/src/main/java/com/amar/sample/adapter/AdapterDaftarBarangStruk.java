...
public class AdapterDaftarBarangStruk extends RecyclerView.Adapter<AdapterDaftarBarangStruk.AdapterPdodukViewHolder> {
    ...
    public void onBindViewHolder(AdapterPdodukViewHolder holder, final int position) {
        ...
        holder.txt_jumlah.setText(dataList.get(position).getItem4()+ " " + dataList.get(position).getItem6());
        ...
        holder.img_hapus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ((ActivityBuatStruk)activity).HapusBarang(dataList.get(position).getItem1(), 1);
            }
        });

        if (position == 0){
            ((ActivityBuatStruk)activity).CliketTourGuide1(holder.ln_bg);
            //((ActivityBuatStruk)activity).CliketTourGuide2(holder.img_hapus);
        }

        holder.ln_bg.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((ActivityBuatStruk)activity).UbahItem(item.getItem10(), item.getItem3(), item.getItem4(), item.getItem6(), item.getItem5(), item.getItem1());
            }
        });
